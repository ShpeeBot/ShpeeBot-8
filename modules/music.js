...
exports.start = (client, options) => {
  try {
    ...
    if (moduleAvailable("ffmpeg-binaries"))
      console.error(
        new Error(
          "[MUSIC] ffmpeg-binaries was found, this will likely cause problems"
        )
      );
    if (
      !moduleAvailable("ytdl-core") ||
      !moduleAvailable("ytpl") ||
      !moduleAvailable("ytsearcher")
    )
      console.error(
        new Error(
          "[MUSIC] one or more youtube specific modules not found, this module will not work"
        )
      );
    ...
    class Music {
      constructor(client, options) {
        ...
        this.clearqueue = {
          enabled:
            options.clearqueue == undefined
              ? true
              : options.clearqueue &&
                typeof options.clearqueue.enabled !== "undefined"
          ...
          exclude: Boolean(
            options && options.clearqueue && options.clearqueue.exclude
          ),
          masked: "clearqueue"
        };
        ...
        this.shuffle = {
          enabled:
            options.shuffle == undefined
              ? true
              : options.shuffle &&
                typeof options.shuffle.enabled !== "undefined"
              ? options.shuffle && options.shuffle.enabled
              : true,
          run: "shuffleFunction",
          alt: (options && options.shuffle && options.shuffle.alt) || [],
          help:
            (options && options.shuffle && options.shuffle.help) ||
            "Shuffle the queue",
          name:
            (options && options.shuffle && options.shuffle.name) || "shuffle",
          usage: (options && options.shuffle && options.shuffle.usage) || null,
          exclude: Boolean(
            options && options.shuffle && options.shuffle.exclude
          ),
          masked: "shuffle"
        };
        ...
        this.anyoneCanSkip =
          options && typeof options.anyoneCanSkip !== "undefined"
        ...
        this.requesterName =
          options && typeof options.requesterName !== "undefined"
            ...
            : true;
        this.inlineEmbeds =
          options && typeof options.inlineEmbeds !== "undefined"
            ? options && options.inlineEmbeds
            : false;
        this.clearOnLeave =
          options && typeof options.clearOnLeave !== "undefined"
            ...
            : true;
        this.messageHelp =
          options && typeof options.messageHelp !== "undefined"
            ? options && options.messageHelp
            : false;
        this.dateLocal = (options && options.dateLocal) || "en-US";
        this.bigPicture =
          options && typeof options.bigPicture !== "undefined"
            ? options && options.bigPicture
            : false;
        this.messageNewSong =
          options && typeof options.messageNewSong !== "undefined"
            ...
            : true;
        this.insertMusic =
          options && typeof options.insertMusic !== "undefined"
            ? options && options.insertMusic
            : false;
      ...
      checkVoice(mem, bot) {
        return new Promise((resolve, reject) => {
          ...
          if (bot.voiceChannel) {
            if (bot.voiceChannel.id == mem.voiceChannel.id)
      ...
      async updatePositions(obj, server) {
        return new Promise((resolve, reject) => {
          ...
          if (obj.working == true)
            reject("The queue is already performing a task!");
          if (server != "000000") {
            obj.working = true;
          ...
          try {
            if (!songs || songs.length <= 0 || typeof obj.songs != "object") {
              if (this.debug)
                console.log(
                  "[MUSICBOT] @updatePositions: songs object was invalid, reseting queue for " +
                    obj.id
                );
      ...
      emptyQueue(server) {
        return new Promise((resolve, reject) => {
          if (!server || typeof server != "string")
            reject(
              "no server id passed or passed obj was no a string @emptyQueue"
            );
          this.queues.set(server, {
            songs: [],
            last: null,
            ...
            if (clear) {
              ...
              if (this.nextPresence !== null) {
                ...
                if (
                  ...
                  ["online", "dnd", "idle", "invisible"].includes(
                    this.nextPresence.status
                  )
                )
                  props.status = this.nextPresence.status;
                if (
                  this.nextPresence.afk &&
                  typeof this.nextPresence.afk == "boolean"
                )
                  props.afk = this.nextPresence.afk;
                if (
                  this.nextPresence.game &&
                  typeof this.nextPresence.game == "string"
                )
                  props.game = { name: this.nextPresence.game };
                else if (
                  this.nextPresence.game &&
                  typeof this.nextPresence.game == "object"
                )
                ...
                client.user
                  ...
                  .catch(res => {
                    console.error(
                      "[MUSICBOT] Could not update presence\n" + res
                    );
                    client.user.setPresence({ game: { name: null } });
                    resolve(client.user.presence);
                  })
    ...
    client.on("ready", () => {
      ...
      if (musicbot.cooldown.exclude.includes("skip"))
        console.warn(
          `[MUSIC] Excluding SKIP CMD from cooldowns can cause issues.`
        );
      if (musicbot.cooldown.exclude.includes("remove"))
        console.warn(
          `[MUSIC] Excluding REMOVE CMD from cooldowns can cause issues.`
        );
      setTimeout(() => {
        if (musicbot.musicPresence == true && musicbot.client.guilds.length > 1)
          console.warn(
            `[MUSIC] MusicPresence is enabled with more than one server!`
          );
      }, 2000);
    });
    ...
    client.on("message", msg => {
      if (msg.author.bot || musicbot.channelBlacklist.includes(msg.channel.id))
        return;
      if (
        musicbot.channelWhitelist.length > 0 &&
        !musicbot.channelWhitelist.includes(msg.channel.id)
      )
        return;
              ...
              if (musicbot.recentTalk.has(msg.author.id)) {
                if (
                  ...
                  return msg.channel.send(
                    musicbot.note(
                      "fail",
                      "You must wait to use music commands again."
                    )
                  );
              }
    ...
    musicbot.playFunction = (msg, suffix, args, ignore) => {
      ...
      let vc = client.voiceConnections.find(
        val => val.channel.guild.id == msg.member.guild.id
      );
      if (vc && vc.channel.id != msg.member.voiceChannel.id)
        return msg.channel.send(
          musicbot.note("fail", `You must be in the same voice channel as me.`)
        );
      if (
        ...
        ytpl(playid, function(err, playlist) {
          ...
          if (playlist.total_items >= 50)
            return msg.channel.send(
              musicbot.note(
                "fail",
                `Too many videos to queue. A maximum of 50 is allowed.`
              )
            );
          var index = 0;
          var ran = 0;
    ...
    musicbot.helpFunction = (msg, suffix, args) => {
      ...
      if (!suffix) {
        if (msg.channel.permissionsFor(msg.guild.me).has("EMBED_LINKS")) {
          const embed = new Discord.RichEmbed();
          embed.setAuthor("Music Commands", client.user.avatarURL);
          embed.setDescription(
            `Use \`${musicbot.botPrefix}${musicbot.help.name} command name\` for help on usage. Anyone with a role named \`${musicbot.djRole}\` can use any command.`
          );
    ...
    musicbot.skipFunction = (msg, suffix, args) => {
      ...
      if (
        voiceConnection &&
        voiceConnection.channel.id != msg.member.voiceChannel.id
      )
        return msg.channel.send(
          musicbot.note("fail", `You must be in the same voice channel as me.`)
        );
                      ...
                      msg.channel
                        ...
                        .then(collected => {
                          ...
                          if (song_number >= 0) {
                            ...
                            if (!musicbot.bigPicture)
                              embed.setThumbnail(
                                `https://img.youtube.com/vi/${videos[song_number].id}/maxresdefault.jpg`
                              );
                            if (musicbot.bigPicture)
                              embed.setImage(
                                `https://img.youtube.com/vi/${videos[song_number].id}/maxresdefault.jpg`
                            ...
                            if (musicbot.requesterName && !resMem)
                              embed.setFooter(
                                `Requested by \`UnknownUser (ID: ${videos[song_number].requester})\``,
                                videos[song_number].requesterAvatarURL
                              );
                            msg.channel
                              .send({
    ...
    musicbot.clearFunction = (msg, suffix, args) => {
      ...
      if (!musicbot.isAdmin(msg.member))
        return msg.channel.send(
          musicbot.note(
            "fail",
            `Only Admins or people with the ${musicbot.djRole} can clear queues.`
          )
        );
      let vc = client.voiceConnections.find(
        val => val.channel.guild.id == msg.member.guild.id
    ...
    musicbot.removeFunction = (msg, suffix, args) => {
      ...
      if (test) {
        ...
        musicbot
          .updatePositions(musicbot.queues.get(msg.guild.id), msg.guild.id)
          .then(res => {
    ...
    musicbot.shuffleFunction = (msg, suffix, args) => {
      ...
      if (q.working == true)
        return msg.channel.send(
          musicbot.note(
            "fail",
            `This servers queue is already performing a task!`
          )
        );
      if (!msg.member.voiceChannel)
        return msg.channel.send(
      ...
      if (musicbot.queues.get(msg.guild.id).songs.length < musicbot.minShuffle)
        return msg.channel.send(
          musicbot.note(
            "fail",
            `Queue must a minimum of ${musicbot.minShuffle} songs to shuffle!`
          )
        );
      if (musicbot.queues.get(msg.guild.id).loop == "song")
        return msg.channel.send(
          musicbot.note("fail", `Cannot shuffle while loop is set to single.`)
        );
      const dispatcher = voiceConnection.player.dispatcher;
      ...
      q.needsRefresh = true;
      musicbot
        .updatePositions(q, msg.guild.id)
        .then(res => {
          q.songs = res.songs;
          musicbot.queues.set(msg.guild.id, q);
        ...
        .catch(res => {
          message.channel.send(
            musicbot.note("fail", "Something went wrong shuffling the queue!")
          );
          console.log("@shuffleFunction " + res);
        });
      ...
      new Promise((resolve, reject) => {
        ...
        if (voiceConnection === null) {
          if (msg.member.voiceChannel && msg.member.voiceChannel.joinable) {
            ...
            !msg.member.voiceChannel.joinable ||
            msg.member.voiceChannel.full
          ) {
            msg.channel.send(
              musicbot.note(
                "fail",
        ...
        .then(connection => {
          ...
          if (
            ...
            if (msg.channel.permissionsFor(msg.guild.me).has("EMBED_LINKS")) {
              const embed = new Discord.RichEmbed()
                ...
                .setFooter(
                  `Requested by ${
                    req !== null ? req.username : "Unknown User"
                  }`,
                  `${req !== null ? req.displayAvatarURL : null}`
                );
              msg.channel.send({ embed });
          ...
          try {
            musicbot.setLast(msg.guild.id, video).then(() => {
              if (musicbot.musicPresence)
                musicbot
                  .updatePresence(queue, msg.client, musicbot.clearPresence)
                  .catch(res => {
                    console.warn(`[MUSIC] Problem updating MusicPresence`);
                  });
            });
              ...
              setTimeout(() => {
                ...
                if (queue.songs.length > 0) {
                  if (loop == "none" || loop == null) {
                    queue.songs.shift();
                    musicbot
                      .updatePositions(queue, msg ? msg.guild.id : 000000)
                      .then(res => {
                        queue.songs =
                          typeof res.songs == "object"
                            ? Array.from(res.songs)
                            : [];
                        musicbot.executeQueue(msg, queue);
                      })
                      .catch(e => {
