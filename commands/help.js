...
exports.run = (client, message, args, level) => {
  ...
  if (!args[0]) {
    ...
    const sorted = myCommands.sort((p, c) =>
      p.help.category > c.help.category ? 1 : -1
    );
    ...
    sorted.forEach(c => {
      ...
      embeds[eN].addField(
        `${c.help.name}${" ".repeat(longest - c.help.name.length)}`,
        c.help.description
      );
      //}
    });
    ...
    if (
      ...
      client.settings.get(message.guild.id).sendHelp === "channel"
    ) {
      ...
      embeds.forEach(e => {
        ...
        message.channel.send({ embed: e }).catch(err => {
          console.error(err);
        });
      });
    } else {
