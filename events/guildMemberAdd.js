...
module.exports = (client, member) => {
  ...
  const welcomeMessage = guildSettings.welcomeMessage.replace(
    "{{user}}",
    member.user.tag
  );
  ...
  if (guildSettings.welcomeEnabled === "true") {
    if (member.guild.channels.find("name", guildSettings.welcomeChannel)) {
      member.guild.channels
        .find("name", guildSettings.welcomeChannel)
        .send(welcomeMessage)
        .catch(e =>
          client.log(
            "log",
            `Unable to send message to welcomeChannel (${guildSettings.welcomeChannel}) on ${member.guild.name} (${member.guild.id}): \n ${e}`,
            "Error"
          )
        );
    }
